...
def generate_and_send_daily_poll(is_test=False):
    ...
    2. СЕКРЕТ КРЕАТИВНЫХ ОТВЕТОВ (РОВНО 12 штук):
       - ГЛАВНОЕ ПРАВИЛО: ГЕНЕРИРУЙ УНИКАЛЬНЫЕ СУЩЕСТВИТЕЛЬНЫЕ И СИТУАЦИИ КАЖДЫЙ ДЕНЬ!
       - 🛑 ЧЕРНЫЙ СПИСОК (ЗАПРЕЩЕНО ИСПОЛЬЗОВАТЬ): кот, бывший, спортзал, пицца, кофе, дедлайн, приложения для знакомств, Grindr. Забудь эти слова!
       - ТОНКОСТЬ (БЕЗ «БОРЩА»): Юмор должен быть с отсылками к гей-культуре (эстетика, поиск партнеров, отношения), но без тупой пошлятины.
       - Как строить ответ: Возьми одну из абстрактных сфер жизни (смотри ниже) и придумай для нее СОВЕРШЕННО НОВУЮ, конкретную ситуацию. Затем свяжи её с темой {selected_holiday} через МЕТАФОРУ. Длина ответа — до 100 символов. В начале ставь подходящий эмодзи.
       
       АБСТРАКТНЫЕ СФЕРЫ ДЛЯ ВДОХНОВЕНИЯ (миксуй их):
       * Работа и карьера (придумывай разные профессии и задачи).
       * Гастрономия (каждый раз генерируй новые блюда, напитки и места).
       * Отношения и романтика (от неловких моментов до семейного быта).
       * Уход за собой и стиль (косметика, одежда, процедуры — придумывай разные!).
       * Домашний быт (ремонт, уборка, хобби, интерьер).
       * Культура и развлечения (кино, музыка, искусство, тусовки, путешествия).
       * Интимная жизнь (метафоричный поиск актива/пассива, флирт, ролевые игры).
