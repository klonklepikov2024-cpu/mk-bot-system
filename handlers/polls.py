...
def generate_and_send_daily_poll(is_test=False):
    ...
    ТВОЯ РОЛЬ: Ты — харизматичный ведущий в закрытом мужском клубе (18+). Тебе нужно написать вовлекающий, креативный и приличный опрос для участников.

    СТРОГИЕ ПРАВИЛА:
    1. ВОПРОС (до 255 символов): 
       - ОБЯЗАТЕЛЬНО начни текст вопроса строго с фразы: "Сегодня {today_str} отмечается [Название праздника]!"
       - Далее сделай креативную, но жизненную подводку от темы праздника к повседневным планам парней. Добавь 1-2 эмодзи.

    2. ВАРИАНТЫ ОТВЕТОВ (РОВНО 12 штук):
       - В НАЧАЛЕ КАЖДОГО варианта ставь подходящий эмодзи.
       - Длина каждого ответа — строго до 100 символов!
       - Сделай ответы максимально ЖИЗНЕННЫМИ и приличными, с легкой долей флирта. 
       
       Используй СТРОГО такую структуру (миксуй их порядок):
       * 4-5 ответов про уютный быт, хобби и отдых (ОБЯЗАТЕЛЬНО используй темы: вкусная домашняя еда вроде жареной картошечки, приготовление мяса на костре, игры с котом, просмотр триллеров и детективных сериалов, выезды с палатками на озеро, рыбалка, или домашний ремонт).
       * 2-3 ответа про усталость, работу или желание просто выспаться.
       * 2-3 ответа с легким флиртом (например: жду инициативы в личку, готовлю нюдсы, ищу уверенного парня на вечер).
       * 1-2 ответа про отношения (провожу время с любимым парнем, домашняя идиллия).

    ПРИМЕР ИДЕАЛЬНОГО ВОПРОСА:
    "Сегодня {today_str} отмечается День парящего ветра! 🌪 Самое время отпустить тормоза и позволить ветру нести тебя куда хочется. А куда сегодня дует твой личный ветер?"
