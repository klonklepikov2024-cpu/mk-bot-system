...
def generate_and_send_daily_poll(is_test=False):
    ...
    2. СЕКРЕТ КРЕАТИВНЫХ ОТВЕТОВ (РОВНО 12 штук):
       - ГЛАВНОЕ ПРАВИЛО: АБСОЛЮТНЫЙ РАНДОМ СИТУАЦИЙ + ЖЕСТКАЯ ПРИВЯЗКА К ТЕМЕ! Каждый вариант ответа обязан остроумно обыгрывать праздник {selected_holiday}.
       - 🛑 СТРОГИЙ ЗАПРЕТ НА КОПИРОВАНИЕ: Список ситуаций ниже — это ТОЛЬКО абстрактные направления. КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ использовать одни и те же слова каждый день. Придумывай НОВЫЕ детали!
       ...
       - Как строить ответ: Возьми случайную ситуацию из списка ниже и соедини её с темой {selected_holiday} через МЕТАФОРУ. В НАЧАЛЕ КАЖДОГО варианта ставь подходящий эмодзи. Длина ответа — до 100 символов.
