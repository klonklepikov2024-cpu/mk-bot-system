...
def show_casino_prizes(message):
    text = (
        ...
        "💎 **СУПЕР-ПРИЗ:** Telegram Premium на 3 месяца!\n"
        "🛍 **СУПЕР-ПРИЗ:** Универсальный Сертификат (1000 ₽)!\n"
...
def process_spin_result(chat_id, username, dice_msg_id, val, uid):
    ...
    if val == 63 and bank_data.get("balance", 0) >= premium_cost_stars:
        ...
        try: bot.send_message(STAFF_GROUP_ID, f"🚨 **ВНИМАНИЕ! СОРВАН СУПЕР-ПРИЗ!** 🚨\n\nПользователь `{uid}` (@{username}) выбил **TELEGRAM PREMIUM**! Фонд казино списан.")
        except Exception as e: logger.error(f"Не удалось уведомить админов о премиуме: {e}")

    elif val == 63: 
        # 1.5 РЕЗЕРВНЫЙ СУПЕР-ПРИЗ (Выпал 63, но в кассе пусто)
        paid_collection.update_one({"uid": uid}, {"$inc": {"bounty_points": 1000, "jackpot_shards": 5}})
        msg = f"🎰 **МИНИ-ДЖЕКПОТ!** 🎰\n\nВы были в миллиметре от Главного Приза, но срываете отличный куш!\n🎁 **Приз:** +1000 Очков Бдительности и 5 Осколков рулетки!"
        pm_msg = f"💎 **Ваш выигрыш:** +1000 Очков и 5 Осколков. Главный приз (Premium) пока копится в фонде казино, попробуйте позже!"
    ...
    if val == 62 and bank_data.get("balance", 0) >= cert_cost_stars:
        ...
        paid_collection.update_one({"uid": uid}, {"$inc": {"bounty_points": 1000, "jackpot_shards": 5}})
        msg = f"🎰 **МИНИ-ДЖЕКПОТ!** 🎰\n\nВы выбили сектор Сертификата, но касса казино пока пуста! Вы забираете резервный куш:\n🎁 **Приз:** +1000 Очков Бдительности и 5 Осколков рулетки!"
        pm_msg = f"💎 **Ваш выигрыш:** +1000 Очков и 5 Осколков. Главный приз (Сертификат) пока копится в фонде казино, попробуйте позже!"
    ...
    # 2. 🎰 ЛЕГЕНДАРНЫЙ ДЖЕКПОТ (7️⃣7️⃣7️⃣ — val: 64)
        ...
        pm_msg = (
            ...
            f"1. Перейдите в [@Elitepost_bot](https://t.me/Elitepost_bot)\n"
    ...
    # 9. 📢 СКИДКИ (Любое кратное 7, кроме уже занятых)
        ...
        elif drop['target'] == 'fine': instruction = "📖 **Как применить:** При обращении в Службу Поддержки за разбаном, администратор выставит вам счет на оплату штрафа. Нажмите кнопку **«🎫 У меня есть промокод»**."
        else: instruction = "📖 **Как применить:** Это универсальный код! Нажмите **«🎫 У меня есть промокод»** при оплате любого штрафа через администратора в боте [@FAQMKBOT](https://t.me/FAQMKBOT), рекламы [@PostGoldBot_bot](https://t.me/PostGoldBot_bot) или VIP-статуса [@Elitepost_bot](https://t.me/Elitepost_bot) ."
    ...
    # 9.5 🌟 ЗАМАСКИРОВАННЫЙ КЭШБЭК (Реальные рубли на баланс — val: 11, 33)
        ...
        if bank_data.get("balance", 0) >= cost_in_stars:
            ...
            # Денег нет: просто выдаем ОЧКИ. Юзер даже не узнает, что мог получить рубли!
            fallback_points = win_rub * 2 # Дадим ему очков х2 от суммы, чтобы выглядело солидно
            paid_collection.update_one({"uid": uid}, {"$inc": {"bounty_points": fallback_points}})
            
            msg = f"💸 **КРУПНЫЙ КУШ!**\n\nБарабан остановился на счастливом числе! Вы выиграли **{fallback_points} очков**!"
            pm_msg = f"💸 Ваш выигрыш из рулетки: +{fallback_points} очков."
