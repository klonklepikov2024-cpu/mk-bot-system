...
def process_ticket_with_ai(uid, user_text, thread_id):
    ...
    try:
        ...
        if user_record and "history" in user_record and len(user_record["history"]) > 0:
            ...
            # 🔥 Оранжевая зона теперь имеет приоритет над "Разбанами" и "Штрафами"
            ...
            # 🔥 Жесткий фикс для слова "СНЯТ" (теперь ищет только как самостоятельное слово)
            elif any(x in meaningful_text for x in ["РАЗБАН", "РАЗМУТ", "АМНИСТИЯ", "УСПЕШНАЯ ВЕРИФИКАЦИЯ", "СНЯТИЕ ОГРАНИЧЕНИЙ"]) or re.search(r'\bСНЯТ\b', meaningful_text):
                ban_type = "clean"
...
1. ВЫСТАВЛЕНИЕ СЧЕТА (АВТО-КАССИР): Если юзер согласен на штраф ("оплачу", "штраф", "буду платить") ИЛИ спрашивает "как купить звезды/где взять звезды" -> СТРОГО выбирай `issue_fine` и обязательно укажи верную сумму штрафа (ДЛЯ ДАННОГО НАРУШЕНИЯ ЭТО: {expected_fine}) в поле `fine_amount`. В `response_text` саркастично похвали его за выбор, А ЕСЛИ ОН СПРАШИВАЛ ПРО ПОКУПКУ ЗВЕЗД — ОБЯЗАТЕЛЬНО ВСТАВЬ ТУДА ШПАРГАЛКУ ИЗ БЛОКА "ИНФОРМАЦИЯ О ЗВЕЗДАХ"! КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО просить писать слово "Готов" на этом этапе!
