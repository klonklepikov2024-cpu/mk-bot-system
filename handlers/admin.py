...
4. ОТКАЗ ОТ КРУЖКА (Штраф 650⭐️): Если отказывается -> СТРОГО выбирай `reply_text`. Выставь ультиматум: штраф 650⭐️ или мут.""" 
            ...
            behavior_rules = """3. ПРОВАЛ ВЕРИФИКАЦИИ / ИГНОР: СТРОГО выбирай `reply_text`. НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ВИДЕО-КРУЖОК! Жестко напомни, что он потратил время впустую. Штраф 500⭐️."""
...
4. ТРЕБОВАНИЕ: СТРОГО выбирай действие `tpl_18` (отправить шаблон). НИ В КОЕМ СЛУЧАЕ не пиши текст вручную и НЕ проси записать кружок на этом этапе! Просто отправь `tpl_18`."""
            ...
            behavior_rules = """3. СПАМ / ФЛУД / БИО: Независимо от того, что пишет юзер (даже если просит верификацию), СТРОГО выбирай `reply_text` или `tpl_flood`. НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ВИДЕО-КРУЖОК! Штраф за спам/ссылки — 500⭐️."""
        ...
        if ban_type in ["basic", "may_1"]:
            circle_tech_info = "\n- 🔑 ВЕРИФИКАЦИЯ: Юзер должен отправить ровно одно слово: «Готов». Только после этого бот выдаст код и таймер!"
            dead_end_rule = "- СРАЗУ скажи юзеру: \"Напиши слово «Готов», чтобы получить секретный код и записать видео-кружок для разбана.\""
        elif ban_type == "orange_zone":
            ...
            dead_end_rule = "- Если юзер должен оплатить штраф: Скажи ему \"Ожидайте счет от администратора\". КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО просить юзера писать слово «Готов» или записывать видео!"
...
2. ДОЖИМ (ПРОДАЖА ШТРАФА): Если юзер спорит, возмущается ("это ошибка", "за что бан?") или не понимает причину -> выбирай `reply_text`. Четко и саркастично объясни, что система не ошибается. Озвучь сумму штрафа за его нарушение и ОБЯЗАТЕЛЬНО закончи сообщение вопросом-крючком: "Будете оплачивать штраф?" или "Выслать вам счет на оплату?".
