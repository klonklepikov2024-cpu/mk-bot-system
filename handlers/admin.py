...
def process_ticket_with_ai(uid, user_text, thread_id):
    ...
    try:
        ...
        if user_record and "history" in user_record and len(user_record["history"]) > 0:
            ...
            # 🔥 НОВОЕ: Универсальный ловец ручных банов (Ловим по эмоциям И по ценнику!)
            ...
            # 👇 НОВОЕ: Ловим Оранжевую зону (молодежь 18-21)
            elif any(x in latest_text for x in ["ОРАНЖЕВАЯ ЗОНА", "18 ЛЕТ", "18-21", "ВОЗРАСТ"]):
...
4. ШТРАФ (650⭐️): Жестко и токсично напомни ему, что он потратил время модераторов впустую и провалил проверку. Бесплатной верификации для него БОЛЬШЕ НЕТ. Единственный способ снять бан — оплатить штраф 650⭐️. Назови точную сумму и спроси, звать ли админа для выдачи кассы."""
        ...
        # 🔥 НОВОЕ: Поведение при ручном жестком бане
        elif ban_type == "manual_hard":
            behavior_rules = """3. ЖЕСТКИЙ РУЧНОЙ БАН: Пользователь был забанен администратором вручную (за отказ от верификации, отказ платить штраф или пререкания). СТРОГО выбирай действие `reply_text`. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО упоминать видео-кружок или слово «Готов»!
4. ИНДИВИДУАЛЬНЫЙ ШТРАФ: Внимательно прочитай причину бана в Досье. Если там указана конкретная цена (например, "Разбан от 1500 звезд" или "2000 звезд") — жестко требуй ИМЕННО ЭТУ СУММУ. Саркастично объясни юзеру, что спорить с администрацией было его главной ошибкой. Если точная сумма в досье не указана, требуй стандартный штраф 1500⭐️. Выставь ультиматум: оплата или вечный бан."""

        # 👇 НОВОЕ: Поведение для Оранжевой зоны (Паспортный контроль по шаблону)
        elif ban_type == "orange_zone":
            behavior_rules = """3. ПАСПОРТНЫЙ КОНТРОЛЬ (18-21 год): Пользователь попал под фильтр контроля молодежи. Для нахождения в сети он обязан документально подтвердить свое совершеннолетие.
4. ТРЕБОВАНИЕ: СТРОГО выбирай действие `send_template` и укажи шаблон `tpl_18`. НИ В КОЕМ СЛУЧАЕ не пытайся генерировать текст вручную через reply_text, просто отправь этот готовый шаблон с запросом паспорта!"""
...
4. ПРОДАЖА (2000⭐️): Если юзер спорит или спрашивает как снять бан -> выбирай `reply_text`. НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ВИДЕО-КРУЖОК! Саркастично высмей его поведение. Штраф за наркотики максимальный — 2000⭐️. Озвучь ровно эту сумму и спроси, звать ли админа для оплаты."""
            ...
            behavior_rules = """3. СИСТЕМНЫЕ НАРУШЕНИЯ (Блок бота, неоплата ВИП, игнор): СТРОГО выбирай `reply_text`. НИ В КОЕМ СЛУЧАЕ НЕ УПОМИНАЙ ВИДЕО-КРУЖОК! Саркастично напомни юзеру о его нарушении (посмотри в досье: если он заблокировал бота или тянул время с оплатой ВИП — высмей именно это). Предложи два пути: базовый штраф за разбан (250⭐️) или покупку тега «Свободен» (650⭐️). Активно продавай тег за 650⭐️."""
