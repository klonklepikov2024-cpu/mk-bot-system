...
def process_ticket_with_ai(uid, user_text, thread_id):
    ...
    try:
        ...
        if user_record and "history" in user_record and len(user_record["history"]) > 0:
            ...
            # 🔥 ВАЖНО: Выделяем капсом УЛИКУ (Доказательство), чтобы ИИ четко её видел! 🔥
            dossier_lines = [f"• {e.get('date', '')} | {e.get('action', '')} | Причина: {e.get('reason', '')} | УЛИКА (ДОКАЗАТЕЛЬСТВО): {e.get('evidence_summary', 'Отсутствует')}" for e in recent]
        ...
        if is_clean:
            ...
            if any(x in meaningful_text for x in ["1 МАЯ", "ПАРАМЕТР", "ФОРМАТ"]):
                ...
                behavior_rules_list.append("- ОШИБКА ФОРМАТА АНКЕТЫ: Объясни, что параметры пишутся строго через слеш (например: 24/180/75). Предложи кружок (бесплатно) ИЛИ покупку иммунитета (650⭐️).")
...
2. ДОЖИМ И ПРЕДОСТАВЛЕНИЕ УЛИК (СПОРИТ ИЛИ ОТРИЦАЕТ): Если юзер отрицает вину ("я не знал", "я не эскорт", "за что?", "коммерция не нужна") и у него есть ШТРАФ -> СТРОГО выбирай `reply_text`! В своем ответе ОБЯЗАТЕЛЬНО ПРОЦИТИРУЙ текст из поля «УЛИКА (ДОКАЗАТЕЛЬСТВО)» из его Досье. Осади его фразами в твоем стиле (например: "Врать, сударь, неприлично, мы всё фиксируем", "Знали или не знали — правила написаны давно, незнание не освобождает от ответственности") и жестко напомни, что нужно оплатить {expected_fine}⭐️.
