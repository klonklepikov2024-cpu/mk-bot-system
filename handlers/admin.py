...
4. ТРЕБОВАНИЕ: СТРОГО выбирай действие `tpl_18` (отправить шаблон). НИ В КОЕМ СЛУЧАЕ не пиши текст вручную и НЕ проси записать кружок на этом этапе! Просто отправь `tpl_18`."""
        ...
        if ban_type in ["basic", "may_1"]:
            ...
            dead_end_rule = "- ВАЖНО: Если пользователь согласен на верификацию, напомни ему, что нужно написать одно слово «Готов» для запуска таймера."
        elif ban_type == "orange_zone":
            ...
            dead_end_rule = "- Если юзер должен оплатить штраф: Скажи ему \"Ожидайте счет от администратора\" (ИЛИ выстави его сам). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО просить юзера писать слово «Готов» или записывать видео!"
