...
def process_ticket_with_ai(uid, user_text, thread_id):
    ...
    try:
        ...
        if user_record and "history" in user_record:
            ...
            # 🔥 ИСПРАВЛЕНИЕ: Ищем причину ТОЛЬКО в самой последней записи, чтобы старые грехи не сбивали скрипт
            ...
            # Проверяем latest_text вместо full_text
            if any(x in latest_text for x in ["ЧЕРНАЯ ЗОНА", "ОРАНЖЕВАЯ ЗОНА", "18 ЛЕТ", "НЕСОВЕРШЕННОЛЕТ", "<18", "ВОЗРАСТ", "ВЕРИФИКАЦИЯ ВОЗРАСТ"]):
                ban_type = "age"
            ...
            elif any(x in latest_text for x in ["КРАСНАЯ ЗОНА", "НАРКОТИКИ", "ЗАПРЕЩЕНКА", "НАРК"]):
                ban_type = "nark"
            elif any(x in latest_text for x in ["ЖЕЛТАЯ ЗОНА", "КОММЕРЦИЯ", "МП", "ПОПРОШАЙ"]):
                ban_type = "commercial"
            elif any(x in latest_text for x in ["СПАМ", "ФЛУД", "РЕКЛАМА", "ЕБАНАТ"]):
                ban_type = "spam"
            elif any(x in latest_text for x in ["БОТ", "VIP", "БТБ", "БВБ", "ТРАНСБОТ", "V БЛОК"]):
                ban_type = "bot_block"
